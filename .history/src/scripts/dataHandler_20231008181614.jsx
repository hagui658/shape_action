...
const HandleShape = ({ shapeData, canvasRef }) => {
  ...
  useEffect(() => {
    ...
    if (shapeData && shapeData.shapeType === "circle") {
      ...
      const xOffset = xInPixels - canvas.width / 2;
      const yOffset = yInPixels - canvas.height / 2;

      const rectX = canvas.width / 2 - widthInPixels / 2 + xOffset;
      const rectY = canvas.height / 2 - heightInPixels / 2 - yOffset;

      console.log(rectX);

      context.beginPath();
      context.rect(rectX, rectY, widthInPixels, heightInPixels);
